...
var lf=atom.packages.getPackageDirPaths()[0]+'/.tvActiveSession.lock';
...
async function startSession () {
  ...
  lockFile.check(lf,{},function(error, isLocked){
    if(error) console.log('checklockError: '+error);
    if(isLocked) return console.log('isLocked: '+error);
    const path = require('path');
    console.log(path.resolve(lf));
    lockFile.lock(lf,{},function(error){
      if(error) console.log('lockError: '+error);
    });

    var openPathOnStart = atom.config.get('textual-velocity.openpath')
...
function stopSession () {
  ...
  if (session) {
    lockFile.unlock(lf,null,function(error){
      if(error) console.log('unlockError: '+error);
    });
    session.dispose()
    session = null
  }
